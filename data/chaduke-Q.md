...
QA2. ``issueOath()`` and ``fund()`` have a dividing-followed-by-multiplication round error, because the ``fund()`` performs a division first to calculate ``rewardPerSecond`` and then   ``issueOath()`` uses a multiplication to calculate the amount ``issuance`` to issue. 
...
Mitigation: we multiple ``rewardPerSecond`` by WAD and then divide ``issuance`` by WAD afterwards, where WAD = 1e18:
